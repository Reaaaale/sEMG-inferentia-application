...
""" Flusso
1. Acquisizione:
   - I dati grezzi vengono letti dalla porta seriale, decodificati e salvati in `shift_buffer` e `raw_buffer`.
...
2. Pre-elaborazione:
   ...
   - Il batch preprocessato e i dati grezzi vengono aggiunti a `data_queue`.

3. Inferenza:
   - I batch preprocessati vengono estratti da `data_queue` per essere processati.
   - Si calcolano la frequenza di predizione e il tempo trascorso tra le predizioni.
...
def acquisition_thread():
    ...
    while not stop_event.is_set():
        try:
            ...
            if decoded_data:
                raw_data_list.append(decoded_data)
                shift_buffer.append(decoded_data) # Aggiungi i dati decodificati al buffer
                raw_buffer.append(decoded_data)  # Salva i dati raw nel buffer parallelo

            if len(shift_buffer) == win_shift_samples:
                buffer.extend(shift_buffer) #aggiungo i nuovi campioni al buffer principale
                shift_buffer.clear() #reset buffer

                if len(buffer) == win_size_samples:
                    raw_batch = pd.DataFrame(list(raw_buffer), columns=['Channel 1', 'Channel 2', 'Channel 3'])  # Dati raw
                    data_frame = pd.DataFrame(buffer, columns=['Channel 1', 'Channel 2', 'Channel 3'])
                    processed_data = preprocess_data(data_frame)
                    if processed_data is not None:
                        data_queue.put((processed_data, raw_batch))  # Aggiungi dati processati e raw
...
def inference_thread():
    model = load_model(MODEL_CONFIG['path']) #carico il modelloà
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    print(f"Modello spostato su {device}")
    last_prediction_time = None #variabile per calcolare quanto tempo è passato tra una predizione e l'altra
    while not stop_event.is_set():
        try:
            if not data_queue.empty():
                processed_data, raw_data_batch = data_queue.get() #se dataqueue non è vuoto prendo i dati preprocessati e raw dal thread di preprocessing
                current_time = time.time()
                #Ciclo per calcolare la frequenza di predizione
...
def main():
    ...
    try:
        ...
        # Salva i dati raw
        if raw_data_list:
            pd.DataFrame(raw_data_list, columns=['Channel 1', 'Channel 2', 'Channel 3']).to_csv("raw_data.csv", index=False)
